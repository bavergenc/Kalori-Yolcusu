    ...
    class HomeFragment : Fragment(), FoodAdapter.OnDeleteClickListener {
        ...
        private fun getAllKcalItemsLiveData() {
            viewModel.getAllKcalItemsLiveData().observe(viewLifecycleOwner) { kcalList ->
                updateProgressBars(kcalList)
            }
        }

        private fun updateProgressBar(value: Int, maxProgress: Int, progressBar: ProgressBar) {
            progressBar.progress = value.coerceAtMost(maxProgress)
        }

        private fun updateProgressBar(value: Double, maxProgress: Int, progressBar: ProgressBar) {
            progressBar.progress = value.toInt().coerceAtMost(maxProgress)
        }

        private fun updateProgressBars(kcalList: List<Kcal>) {
            if (kcalList.isNotEmpty()) {
                ...
                binding.caloriprogress.progress = totalCalories.coerceAtMost(maxProgressCalorie)
                binding.carbprogressBar.progress = totalCarbohydrates.toInt().coerceAtMost(maxProgressCarb)
                binding.proteinProgressBar.progress = totalProtein.toInt().coerceAtMost(maxProgressProtein)
                binding.fatprogress.progress = totalFat.toInt().coerceAtMost(maxProgressFat)
            } else {
